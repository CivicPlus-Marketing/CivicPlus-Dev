...
let BudTsConfigValues = class BudTsConfigValues extends Extension {
    ...
    async register(bud) {
        ...
        if (this.tsConfigSource?.include)
            this.setInclude(tsConfig.include);
        if (this.tsConfigSource?.exclude)
            this.setExclude(tsConfig.exclude);
        if (this.tsConfigSource?.bud)
            this.setBud(tsConfig.bud);
        if (this.bud?.useCompilerOptions === true)
            this.enable();
    }
    get tsConfigSource() {
        return (this.app.context.files[`tsconfig.json`]?.module ??
