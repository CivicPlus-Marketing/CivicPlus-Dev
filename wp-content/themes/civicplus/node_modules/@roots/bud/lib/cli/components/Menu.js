...
export const Menu = ({ cli }) => {
    ...
    return (_jsx(Box, { flexDirection: "column", marginTop: 1, children: options.map(([option, description, command], index) => {
            return (_jsxs(Text, { color: selected === index ? `blue` : `white`, children: [selected === index ? figures.radioOn : figures.radioOff, `  `, option, _jsxs(Text, { color: "white", dimColor: true, children: [` `, description] })] }, index));
        }) }));
};
