...
const instantiateServices = (app) => async (signifier) => {
    ...
    const label = service.label ?? service.constructor?.name
        ...
        : signifier;
    app[label] = service;
    app.log(chalk.blue(`bud.${label}`), figures.arrowLeft, chalk.cyan(!isString(signifier) ? `[object]` : signifier));
...
export const bootstrap = async function () {
    ...
    await Promise.all([...this.context.services]
        ...
        .map(instantiateServices(this))).catch(error => {
    ...
    this.hooks
        .fromMap({
        'location.@dist': isString(this.context.output) && this.context.output !== ``
            ? this.context.output
            : `dist`,
        ...
        'location.@src': isString(this.context.input) && this.context.input !== ``
            ? this.context.input
            : `src`,
        ...
        .when(this.isDevelopment, ({ hooks }) => hooks.fromMap({
        'dev.middleware.enabled': [`dev`, `hot`],
    }));
    Object.entries(lifecycle).map(([eventHandle, callbackName]) => [...this.services]
    ...
    await [`bootstrap`, `register`, `boot`].reduce(async (promised, event) => {
        await promised;
