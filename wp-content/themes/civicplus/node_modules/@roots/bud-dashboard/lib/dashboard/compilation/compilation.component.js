...
const Compilation = ({ compilation, context, displayAssets, displayEntrypoints, }) => {
    if (!compilation)
        return null;
    ...
    return (_jsxs(Ink.Box, { flexDirection: "column", children: [_jsxs(Ink.Box, { flexDirection: "row", children: [_jsxs(Ink.Text, { dimColor: true, children: [figures.lineDownRightArc, figures.line] }), _jsx(Ink.Text, { color: colorFromStats(compilation), children: compilation.errorsCount > 0 ? ` ${figures.cross}` : `` }), _jsx(Ink.Text, { children: ` ` }), _jsx(Ink.Text, { children: compilation.name }), _jsx(Ink.Text, { children: ` ` }), compilation.outputPath ? (_jsxs(Ink.Text, { color: color.blue, children: ["./", relative(context.basedir, compilation.outputPath)] })) : null, _jsx(Ink.Text, { children: ` ` }), _jsxs(Ink.Text, { dimColor: true, children: ["[", compilation.hash, "]"] })] }), !compilation.isChild ? (_jsxs(_Fragment, { children: [_jsx(Ink.Text, { dimColor: true, children: figures.lineVertical }), _jsx(Ink.Box, { flexDirection: "column", children: entrypoints.some(({ assets }) => assets?.length > 0) ? (_jsxs(Ink.Box, { flexDirection: "column", children: [_jsx(Title, { children: _jsx(Ink.Text, { color: colorFromStats(compilation), dimColor: displayEntrypoints === false, children: "entrypoints" }) }), displayEntrypoints
                                    ? entrypoints
                                        .filter(({ assets }) => assets.length > 0)
                                        .map((chunk, id) => (_jsx(Ink.Box, { flexDirection: "column", children: _jsx(ChunkGroup, { indent: [true], ...chunk, final: id === entrypoints.length - 1, minWidth: longestEntrypointAssetLength }) }, id)))
